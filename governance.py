...
def _mock_policy_check(entities: dict) -> dict:
    ...
    if price is not None and price > policies["budget_limit"]:
        ...
        _log("📋", "Senso", f"Mock policy VIOLATION — {result['violation']}", _YELLOW)
        return result
    ...
    _log("📋", "Senso", "Mock policy: COMPLIANT", _CYAN)
...
def _keyword_safety_check(thought: str) -> dict:
    ...
    _log("🛡️", "Modulate", f"Keyword safety: {label}", color)
    return result
